...
user_certificate_pass = "pk password"
...
def get_message(update, context) :
    ...
    for u in temp:
        print(u.message)
    res = bot.sendMessage(chat_id='-1001490131647', text=update.channel_post.text+" >> I'm bot") #@channel로 메세지를 보냅니다.
    print(res)
    return

def worker(tempDate,isToday, loop):
    asyncio.set_event_loop(loop)
    loop.run_until_complete(queryProfit(tempDate,isToday))
...
async def queryProfit(tempDate='', isToday = True):
    ...
    tmpStr = '총매수: ' + str(totmssum) + ' 총매도: ' + str(totmdsum) + ' 총수익: ' + format(
        (float(totmdsum) * 0.9974 - float(totmssum)) / float(totmssum), "10.2%")
    print(tmpStr)
    sendTeleMsg(tmpStr)
    pythoncom.CoUninitialize()
...
def help_command(update, context) :
    ...
    if len(context.args) == 0:
        ...
        # loop.run_until_complete(queryProfit(tempDate, True))
        # loop.close()
        loop = asyncio.new_event_loop()
        p = threading.Thread(target=worker, args=(tempDate, True, loop,))
        p.start()
        p.join()
