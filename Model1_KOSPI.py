...
if __name__ == "__main__":
    ...
    now = time.localtime()
    myPrint.fnDebug = '[KOSPI] log_' + "%04d%02d%02d" % (now.tm_year, now.tm_mon, now.tm_mday) + '.txt'
    ...
    def T1452(idx=0, isToday=1):
        ...
        instXAQueryT1452.SetFieldData("t1452InBlock", "gubun", 0, "1") # 1: KOPSPI 2:KOSDAQ
    ...
    def strategy2(_shcode, tempDate,refname=''):
        ...
        if rsiRes == 2 or rsiRes == 3:
            ...
            tmpStr = "KOSPI 종목: "+ refname+ " 포착시각 : "+ str(now.tm_hour)+ ":"+ str(now.tm_min) # +" 현재가: "+str(tdf8412["종가"].values[-1])
            print(tmpStr)
    ...
    def strategy1(_shcode, tempDate, msrate, bidrem1, offerrem1, price, skip=False, refname=''):
        ...
        prognetbuy = 0
        if skip == False :
            df = orderManager.t1636(구분="0",종목코드=_shcode)
        ...
        if skip == True or prognetbuy > 0 : # float(df["등락률"].values[0]) > 0 and float(df["순매수금액"].values[0]) > 0:
            ...
            if int(close15mma )> int(close60dma):
                tdf8412 = orderManager.t8412(단축코드=_shcode, 단위="10", 요청건수="10", 시작일자="", 종료일자="99999999", cts_date="",
                                             comp_yn="N")
                if tdf8412.shape[0] == 0:
                    print('[ERR] 8412 주식차트(10분) 조회 불가')
                    return
                ...
                if rsiRes == 2 or rsiRes == 3:
                    ...
                    beepsound(freq=500, dur=500)
                    beepsound(freq=1500, dur=500)
                    now = time.localtime()
    ...
    def searchCandidates(condtion=1):
        ...
        if condtion == 1:
            ...
            df1, df2 = orderManager.t1825(검색코드="6412", 구분="1") #양음양 6006 # 거래량 6412
            ...
            df1, df2 = orderManager.t1825(검색코드="6306", 구분="1") #프로그램순매수 100
            for i in range(0, df2.shape[0]):
    ...
    while True:
        # botManager.queryMine(isToday=False,date='20201211')
        f = Figlet(font='slant')
        now = time.localtime()
        ...
        # strategy2("078930", "20201227", refname="GS")
